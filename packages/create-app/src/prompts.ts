...
export async function runPrompts(): Promise<ScaffoldSelections> {
  ...
  while (projectName.length < 2) {
    ...
    if (projectName.length < 2) console.log("  Name must be at least 2 characters.");
  }
  ...
  const db = await selectOne<DbProvider>(
    "Database",
    [
      { value: "firebase", label: "Firebase Firestore", hint: "NoSQL, serverless" },
  ...
  const auth = await selectOne<AuthProvider>(
    "Authentication",
    [
      { value: "firebase", label: "Firebase Auth", hint: "OAuth, email, phone" },
      { value: "nextauth", label: "NextAuth v5",    hint: "Flexible, many providers" },
  ...
  const email = await selectOne<EmailProvider>(
    ...
    [
      ...
      { value: "nodemailer", label: "Nodemailer",  hint: "SMTP, any mail server" },
  ...
  const storage = await selectOne<StorageProvider>(
    "File Storage",
    [
      { value: "firebase",    label: "Firebase Storage", hint: "GCS-backed, serverless" },
      { value: "s3",          label: "AWS S3 / R2",       hint: "S3-compatible buckets" },
      { value: "cloudinary",  label: "Cloudinary",        hint: "Media transforms + CDN" },
      { value: "uploadthing", label: "UploadThing",       hint: "Simple file uploads" },
    ],
    "firebase",
  );
  ...
  const css = await selectOne<CssProvider>(
    "CSS framework",
    [
      { value: "tailwind", label: "Tailwind CSS",    hint: "Utility-first, recommended" },
      { value: "vanilla",  label: "Vanilla CSS vars", hint: "Pure CSS custom properties" },
    ],
    "tailwind",
  );
  ...
  const search = await selectOne<SearchProvider>(
    ...
    [
      ...
      { value: "typesense",   label: "Typesense",   hint: "Self-hosted / Typesense Cloud" },
      { value: "meilisearch", label: "Meilisearch", hint: "Self-hosted, open source" },
    ],
    "none",
  );
  ...
  const payment = await selectOne<PaymentProvider>(
    ...
    [
      { value: "none",     label: "None" },
      { value: "razorpay", label: "Razorpay", hint: "India-first, UPI + cards" },
  ...
  const featureValues = await selectMany<string>(
    "Features to install",
    ALL_FEATURES.map((f: FeatureDef) => ({ value: f.key, label: f.label, hint: f.hint })),
  ...
  return {
    ...
    features: new Set(featureValues.length > 0 ? featureValues : ["layout", "forms"]),
    targetDir,
  };
}
